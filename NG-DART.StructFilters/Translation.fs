...
type MsgHdrToken =
    ...
    | StructOrUnionStart of stOrUnName: string * stIsUnion: bool
    | StructOrUnionEnd
    | BoolOperator
...
let parseStructOrUnionStart str =
    let m = RegexParsers.structOrUnionStart.Match str
    if m.Success then 
        let isUnion = m.Groups.[1].Value = "union"
        Some <| StructOrUnionStart (m.Groups.[2].Value, isUnion)
    else None
...
let shouldIncludeToken x =
    match x with 
    | Whitespace -> false
    | FieldPrimitive (_, name) ->
        // Filter out pads and spares.
        let isPadOrSpare name = 
            let m = RegexParsers.padOrSpareFieldName.Match name
            m.Success
        not (isPadOrSpare name)
    | _ -> true
...
let rec getFields fieldName structName theParentType structToks =
    let rec loop parentType acc lst =
        match lst with
        | [] -> { acc with Fields = acc.Fields |> List.rev }
        | StructOrUnionStart (name, isUnion) :: t when name = structName -> 
            let fd = FieldData.create fieldName name theParentType isUnion // for a parent struct, the name is the type
        ...
        | FieldPrimitive (typ, name) :: t ->
            ...
            | "" ->
                loop parentType acc t
            | _ ->
                let fd = FieldData.create name typ parentType false
                let fld = { Data = fd; Fields = [] }
        ...
        | FieldStruct (typ, name) :: t ->
            ...
            | "" ->
                loop parentType acc t
            | _ ->
                let fld = getFields name typ parentType structToks
                loop parentType { acc with Fields = fld :: acc.Fields } t
        | _ :: t ->
                loop parentType acc t
...
let private getStructDef headerFileData structName =
    ...
    |> getFields structName structName ""
